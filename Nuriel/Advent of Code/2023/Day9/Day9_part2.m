...
fid = fopen('D:\Home Projects\GitHub Projects\aoc-2023\Nuriel\Advent of Code\2023\Day9\input.txt','r');
format = '%c';
...
for i = 1:length(doc)
    ...
    for j = 2:length(list(i).decim(1).derivative)
        ...
        if list(i).decim(j).derivative == zeros(length(list(i).decim(j).derivative),1)
            ...
            list(i).decim(j).sub = 0;
            for k = 1:maxList-1
                list(i).decim(j-k).add = list(i).decim(j-k).derivative(end) + list(i).decim(j-k+1).add;
                list(i).decim(j-k).sub = list(i).decim(j-k).derivative(1) - list(i).decim(j-k+1).sub;
            end
            break;
        end
    end
    list(i).added = list(i).input(end) + list(i).decim(1).add;
    list(i).subtracted = list(i).input(1) - list(i).decim(1).sub;
    answer1 = answer1 + int32(list(i).added);
    answer2 = answer2 + int32(list(i).subtracted);
end