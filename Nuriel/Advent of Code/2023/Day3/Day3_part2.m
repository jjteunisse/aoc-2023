...
Numbers.coordNumXY(:,1) = mod(Numbers.coordNum-1,length(M(1,:)))+1;
Numbers.coordGearXY(:,1) = mod(Numbers.coordGear-1,length(M(1,:)))+1;
...
Numbers.coordNumIdIncl = zeros(1,2);
counter = 1;
answer = 0;
for i = 1:length(Numbers.coordGear)
    temp = abs(Numbers.coordNumXY - Numbers.coordGearXY(i,:));
    pair = unique(Numbers.coordNumId(find((temp(:,1) <= 1) & (temp(:,2) <= 1))));
    if length(pair) == 2
        Numbers.coordNumIdIncl(counter,:) = pair;
        answer = answer + str2num(Numbers.combined(pair(1)).Id) * str2num(Numbers.combined(pair(2)).Id);
        counter = counter + 1;
    end
end